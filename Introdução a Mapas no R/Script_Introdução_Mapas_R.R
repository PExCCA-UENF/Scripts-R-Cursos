...
# install.packages('sf', dependencies = T)   # instalando o pacote 'sf'.
...
tab.sf3 <- sf::st_transform(x = tab.sf2,
...
# Antes de iniciar, vamos criar um RProject para arquivar todos os arquivos referentes ao curso.
# Para criar um projeto, clique em File > New Project > New Directory > New Project.
# Agora podemos criar pastas e realizar o donwload diretamente para elas.
...
url_file <- "https://terrabrasilis.dpi.inpe.br/download/dataset/legal-amz-aux/vector/conservation_units_legal_amazon.zip"
...
download.file(url = url_file,
              destfile = "Arquivos/UCAmazoniaLegal.zip")

# A função unzip() permite descompactar o arquivo zip.
unzip(zipfile = "Arquivos/UCAmazoniaLegal.zip",   # Arquivo que será executado.
      exdir = "Arquivos/UCAmazoniaLegal/")     # O diretório que será extraído os arquivos.

dir("Arquivos/UCAmazoniaLegal/")  # Lista os arquivos do dest_file2.

# Para ler os arquivos do tipo shapefile (.shp) vamos usar a função st_read() do pacote 'sf'.
UC <- sf::st_read(dsn = "Arquivos/UCAmazoniaLegal/conservation_units_legal_amazon.shp")
...
## Para isso, podemos utilizar as funções do pacote tidyverse
...
UC.Mod <-
  UC |>
  mutate(
    ano_cria = parse_number(ano_cria) # Extrai a primeira informação numérica da coluna
  )
...
UC.Fil <-
  UC.Mod |>
  filter(
    ano_cria >= 2000 # Filtra os dados, referentes ao ano_cria maior ou igual a 2000
  )

# Podemos exportar dados shapefile usando a função st_write() do pacote 'sf'.
sf::st_write(UC.Fil,                # Objeto que será exportado.
             'Arquivos/UCs_2000_2024.shp')   # Nome e formato do arquivo que será exportado.
...
categorias <- unique(UC.Fil$categoria)
...
mapaBR.UC <- ggplot() +
  ...
  geom_sf(data = UC.Fil,                   # Dados das Unidades de Conservação na Amazônia Legal.
          aes(fill = categoria)) +
